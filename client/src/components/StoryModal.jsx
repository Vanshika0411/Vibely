...
const StoryModal = ({ setShowModal, fetchStories }) => {
  ...
  const handleMediaUpload = (e) => {
    ...
    if (file.type.startsWith("video")) {
      if (file.size > MAX_VIDEO_SIZE_MB * 1024 * 1024) {
        toast.error(`Video file can't exceed ${MAX_VIDEO_SIZE_MB} MB.`);
        setMedia(null);
        setPreviewUrl(null);
        return;
      }
      ...
      video.onloadedmetadata = () => {
        ...
        if (video.duration > MAX_VIDEO_DURATION) {
          ...
          setMedia(file); // ✅ media state set
          setPreviewUrl(URL.createObjectURL(file)); // ✅ preview set
          setText("");
          setMode("media");
        }
      };
  ...
  const handleCreateStory = async () => {
    ...
    if (media) formData.append('media', media); // only append if exists
