...
const RecentMessages = () => {
  ...
  const fetchRecentMessages = async () => {
    try {
      ...
      const { data } = await api.get("/api/message/me", {
        headers: { Authorization: `Bearer ${token}` },
      });

      if (data.success) {
        // Group messages by sender ID, ensuring the most recent one is picked
  ...
  useEffect(() => {
    if (user) {
      fetchRecentMessages();
      const interval = setInterval(fetchRecentMessages, 30000); // Refresh every 30 seconds
